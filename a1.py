...
class GameBoard:
    ...
    """
    # _characters: List[Any]  # List[List[Optional[Character]]]]
    ...
    def place_character(self, c: Character) -> None:
        ...
        if isinstance(c, Player):
            self._player = c
        self._characters[c.y][c.x].append(c)
        ...
        for y in range(self.height):
            ...
            for x in range(self.width):
                ...
                if len(lst) == 0:
                    ...
                    role = lst[0]
                    if isinstance(role, Raccoon):
                        row.append('R')
                    elif isinstance(role, SmartRaccoon):
                        row.append('S')
                    elif isinstance(role, Player):
                        row.append('P')
                    elif isinstance(role, GarbageCan):
                        if role.locked:
                            row.append('C')
                        else:
                            row.append('O')
                    elif isinstance(role, RecyclingBin):
                        row.append('B')
            grid.append(row)
        return grid
    ...
    def check_game_end(self) -> Optional[int]:
        ...
        for j in range(self.height):
            for i in range(self.width):
                characters = self.at(i, j)
                if len(characters) > 1:
                    raccoon_target += 1
                else:
                    if len(characters) == 1 \
                            and isinstance(characters[0], Raccoon):
                        raccoon_count += 1
                        if characters[0].check_trapped():
                            raccoon_target += 1

        # print(f'{raccoon_count}=={raccoon_target}')
        if raccoon_count == raccoon_target:
            self.ended = True
            return 11
        else:
            self.ended = False
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        True
        """
        bin_x = self.x + direction[0]
        bin_y = self.y + direction[1]
        if not self.board.on_board(bin_x, bin_y):
            return False

        characters = self.board.at(bin_x, bin_y)
        if len(characters) > 1:
            return False

        if len(self.board.at(bin_x, bin_y)) == 0:
            self.board.at(self.x, self.y).remove(self)
            self.x = bin_x
            self.y = bin_y
            self.board.at(self.x, self.y).append(self)
            return True
        else:
            rb = characters[0]
            if isinstance(rb, RecyclingBin) and rb.move(direction):
                self.board.at(self.x, self.y).remove(self)
                self.x = bin_x
                self.y = bin_y
                self.board.at(self.x, self.y).append(self)
...
class Player(TurnTaker):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        True
        """
        player_x = self.x + direction[0]
        player_y = self.y + direction[1]
        if not self.board.on_board(player_x, player_y):
            return False

        characters = self.board.at(player_x, player_y)
        if len(characters) == 0:
            self.board.at(self.x, self.y).remove(self)
            self.x = player_x
            self.y = player_y
        ...
        if isinstance(character, RecyclingBin) and character.move(direction):
            self.board.at(self.x, self.y).remove(self)
            self.x = player_x
            self.y = player_y
            self.board.at(self.x, self.y).append(self)
            ...
            return True
        elif isinstance(character, GarbageCan) and not character.locked:
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        for direction in DIRECTIONS:
            ...
            if self.board.on_board(x_d, y_d):
                characters = self.board.at(x_d, y_d)
                if len(characters) == 0:
                    return False
                elif len(characters) == 1 \
                        and isinstance(characters[0], GarbageCan):
                    return False
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        True
        """
        rac_x = self.x + direction[0]
        rac_y = self.y + direction[1]
        if not self.board.on_board(rac_x, rac_y):
            return False

        characters = self.board.at(rac_x, rac_y)
        if len(characters) == 0:
            self.board.at(self.x, self.y).remove(self)
            self.x = rac_x
            self.y = rac_y
        ...
        character = characters[0]
        if isinstance(character, GarbageCan) and character.locked is False:
            self.board.at(self.x, self.y).remove(self)
            self.x = rac_x
            self.y = rac_y
            self.board.at(self.x, self.y).append(self)
            return True
        elif isinstance(character, GarbageCan) and character.locked:
    ...
    def take_turn(self) -> None:
        ...
        (2, 1)
        """
        for direct in get_shuffled_directions():
            if self.move(direct):
                return
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        for direct in DIRECTIONS:
            this_x = self.x + direct[0]
            this_y = self.y + direct[1]
            if self.board.on_board(this_x, this_y):
                lst = self.board.at(this_x, this_y)
                n = 0
                while len(lst) == 0 or isinstance(lst[0], Player):
                    this_x += direct[0]
                    this_y += direct[1]
                    lst = self.board.at(this_x, this_y)
                    n += 1
        ...
        if len(direction) == 0:
            ...
            i = 0
            while not distance[i] == min_dist:
                i += 1
            correct_direct = direction[i]
            self.x += correct_direct[0]
            self.y += correct_direct[1]
